...
local function viewRoll()
	...
	if displayplayer.mo.skin != "kombifreeman" return end -- Make sure we're playing as Freeman
	if (maptol & TOL_NIGHTS) then return end -- let them handle it
	if camera.chase == true return end -- ONLY run this for non-chase camera!!
	...
	player.hl1wepbob = $>>2
	if player.hl1wepbob > FRACUNIT*16
...
local function writeConfig()
	...
	local file = io.openlocal("client/qvr_autorun.cfg", "w")
...
local function readConfig()
	local file = io.openlocal("client/qvr_autorun.cfg", "r")
	if file == nil then return end
...
convars[#convars+1] = CV_RegisterVar({
	name = "cl_rollspeed",
	defaultvalue = "36", -- Quake cl_rollspeed. 200 is Quake's run speed, 36 is SRB2's (default) run speed
...
addHook("PlayerSpawn", function(player)
	-- avoid debug flood
	if player == displayplayer then displayplayer.v_dmg_roll = 0; displayplayer.v_dmg_time = 0 end
end)
